...
{
    internal readonly record struct WatchEventContext(
        Type ResourceType,
        int WatchIndex,
    ...
    private readonly Func<WatchEventContext, CancellationToken, Task>? _afterWatchEventAsync;
    private readonly ConcurrentDictionary<Type, int> _watchCounts = [];
        ...
        Func<WatchEventContext, CancellationToken, Task>? afterWatchEventAsync = null)
    {
        ...
        _allocateServiceAddresses = allocateServiceAddresses;
    ...
    public async IAsyncEnumerable<(WatchEventType, T)> WatchAsync<T>(string? namespaceParameter = null, [EnumeratorCancellation] CancellationToken cancellationToken = default) where T : CustomResource, IKubernetesStaticMetadata
    {
        ...
        var watchIndex = _watchCounts.AddOrUpdate(typeof(T), 1, static (_, count) => count + 1);
        ...
        _watchStarted?.Invoke(typeof(T), watchIndex);
        ...
        try
        {
            while (true)
            {
                ...
                if (res is T tRes)
                {
                    var context = new WatchEventContext(typeof(T), watchIndex, evtType, res);
                    if (_beforeWatchEventAsync is not null)
                    {
                        await _beforeWatchEventAsync(context, cancellationToken);
                    }
                    ...
                    {
                        if (_afterWatchEventAsync is not null)
                        {
                            await _afterWatchEventAsync(context, cancellationToken);
                        }
                    }
                }
            }
